...
def ral_calc(query,client,message):
    ...
    if len(splitted) > 1:
        region = str(splitted[1].lower())
    else:
        region = "lombardia"
    url = "https://www.pmi.it/servizi/292472/calcolo-stipendio-netto.html?step=2&ral=" + ral + "&reg=" + region + "&com=0.8&car=no&emp=privato&hw=no&toc=ind&tow=no&child_noau=0&child_au=0&childh=0&childcharge=100&family=0&monthlypay=13&days=365"
